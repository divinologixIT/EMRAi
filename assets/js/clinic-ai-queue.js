(() => {
  ...
  const selectCurrentPatient = (patient) => {
    ...
    document.getElementById("activePatientTime").textContent = patient.arrived;
    document.getElementById("activePatientName").textContent = patient.name;
    document.getElementById("activePatientMeta").textContent = `${patient.meta} · ${patient.token}`;
...
(() => {
  const doctorList = document.querySelector(".clinic-doctor-list");
  const doctorCards = [...document.querySelectorAll(".clinic-doctor-card")];
  ...
  const activeBanner = document.querySelector(".active-consultation-banner");
  const queueMetrics = document.querySelector(".metric-grid");

  if (queueLayout && queueCard && activeBanner && queueMetrics) {
    const queueColumn = document.createElement("div");
    queueColumn.className = "clinic-queue-column";
    queueLayout.insertBefore(queueColumn, queueCard);
    queueColumn.append(activeBanner, queueMetrics, queueCard);
  }

  doctorCards.forEach((card) => {
    card.addEventListener("click", () => {
      doctorCards.forEach((item) => item.classList.toggle("active", item === card));
      selectedDoctorName.textContent = card.dataset.doctor;
      toastMessage.textContent = `${card.dataset.doctor}'s live queue loaded`;
      toast.classList.add("show");
      window.setTimeout(() => toast.classList.remove("show"), 2400);
    });
  });

  document.querySelector('[aria-label="Previous doctors"]')?.addEventListener("click", () => {
    const step = (doctorCards[0]?.getBoundingClientRect().width || 220) + 10;
    doctorList.scrollBy({ left: -step, behavior: "smooth" });
  });

  document.querySelector('[aria-label="Next doctors"]')?.addEventListener("click", () => {
    const step = (doctorCards[0]?.getBoundingClientRect().width || 220) + 10;
    doctorList.scrollBy({ left: step, behavior: "smooth" });
  });
})();
